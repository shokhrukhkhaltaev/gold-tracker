  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <span className="font-label-bold">{selected || 'Город'}</span>
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 w-52 bg-white rounded-xl shadow-card-hover border border-outline-variant z-40 overflow-hidden max-h-64 overflow-y-auto">
          {cities.map(city => (
            <button
              key={city}
              ...
              className={`w-full text-left px-4 py-3 font-label-bold text-label-bold hover:bg-surface-container-low transition-colors ${
                city === selected ? 'text-primary bg-primary-container/10' : 'text-on-surface'
              }`}
            >
              {city}
            </button>
