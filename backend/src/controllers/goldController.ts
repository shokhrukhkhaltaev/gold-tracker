...
import { sendDailyPrices, handleUpdate, registerWebhook } from '../services/telegramService.js';

export async function getPrices(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const result = await goldService.getPrices();
    res.json({ data: result.prices, updatedAt: result.updatedAt, isMockData: result.isMockData });
  } catch (err) { next(err); }
}
...
export async function getPriceHistory(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (isNaN(weightGrams) || weightGrams <= 0) { res.status(400).json({ error: 'Invalid weight parameter' }); return; }
    const history = await goldService.getPriceHistory(weightGrams);
...
export async function getCities(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const cities = await goldService.getCities();
...
export async function getBanks(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (weightGrams !== undefined && isNaN(weightGrams)) { res.status(400).json({ error: 'Invalid weight parameter' }); return; }
    const result = await goldService.getBanksWithAvailability(city, weightGrams);
    res.json({ data: result.banks, updatedAt: result.updatedAt, isMockData: result.isMockData });
  } catch (err) { next(err); }
}

export async function getStatus(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    res.json(goldService.getStatus());
  } catch (err) { next(err); }
}
...
export async function sendTelegramTest(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    res.json({ message: 'Telegram prices sent' });
  } catch (err) { next(err); }
}

export async function telegramWebhook(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
